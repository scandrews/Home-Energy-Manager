...
/*
    switch (whatToDo){
      ...
      case 8:
          ...
          minHouseTemp = atoi (strtok(packetBuffer, " "));
...
/*
    char flags[100];
    sprintf(flags, "%d:%d:%d:%d:%d", whichSensor, redState, recircMotorState, furnaceState, maxHouseTemp);
...
//  READ TEMPERATURES
   // read analog values and convert to centagrate
    ...
    sensorInput = analogRead(sensorPin6);    //read the analog sensor and store it
    voltage = sensorInput * 5.0;             //find percentage of input reading
    voltage /= 1024;
